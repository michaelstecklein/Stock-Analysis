...
def __update_RSI_entry(ticker, date):
	# Get previous values
	prev = __get_prev_rsi(ticker, date)
	if prev != None:
		prev_rsi = prev[0]
...
def __update_RSI():
	#__create_rsi_table()
	result = query("SELECT ticker, date FROM {} WHERE RSI={} ORDER BY date ASC;".format(__RSI_TABLE_NAME,DEFAULT_INDICATOR_VALUE))
	for row in result:
		ticker = row[0]
		date = row[1]
		log("Updating {} {} RSI".format(ticker, date))
		__update_RSI_entry(ticker,date)
